...
# Backward-compatible wrappers used by tests and older helper code.
run_db_sql() {
  local sql="$1"
  run_target_db_sql "$sql"
}

run_db_scalar() {
  local sql="$1"
  run_target_db_scalar "$sql"
}
...
resolved_service_user() {
  if [[ -n "${ODOO_SERVICE_USER:-}" ]]; then
    printf '%s\n' "$ODOO_SERVICE_USER"
  elif is_root_user; then
    printf 'odoo\n'
  else
    id -un
  fi
...
resolved_service_group() {
  if [[ -n "${ODOO_SERVICE_GROUP:-}" ]]; then
    printf '%s\n' "$ODOO_SERVICE_GROUP"
  elif id "$(resolved_service_user)" >/dev/null 2>&1; then
    id -gn "$(resolved_service_user)"
  else
    resolved_service_user
  fi
}
...
ensure_service_account() {
  ...
  run_privileged groupadd --system "$service_group" >/dev/null 2>&1 || true
  fi

  if ! id "$service_user" >/dev/null 2>&1; then
  login_shell="$(select_nologin_shell)"
  run_privileged useradd --system --gid "$service_group" --home-dir "$ROOT" --shell "$login_shell" "$service_user" >/dev/null 2>&1 || \
  run_privileged useradd --system --gid "$service_group" --shell "$login_shell" "$service_user" >/dev/null 2>&1 || \
...
prepare_service_runtime_permissions() {
  ...
  run_privileged mkdir -p "$DATA_DIR" "$log_dir" 2>/dev/null || true
  run_privileged touch "$LOG_FILE" 2>/dev/null || true

  run_privileged chown "$service_user:$service_group" "$ROOT" "$log_dir" 2>/dev/null || true
  run_privileged chmod 750 "$ROOT" "$log_dir" 2>/dev/null || true
  run_privileged chown -R "$service_user:$service_group" "$DATA_DIR" 2>/dev/null || true
  run_privileged chown "$service_user:$service_group" "$LOG_FILE" "$odoo_conf" 2>/dev/null || true
  run_privileged chmod 640 "$LOG_FILE" "$odoo_conf" 2>/dev/null || true

  if [[ -n "${CUSTOM_ADDONS_DIR:-}" && -d "$CUSTOM_ADDONS_DIR" ]]; then
  run_privileged chown -R "$service_user:$service_group" "$CUSTOM_ADDONS_DIR" 2>/dev/null || true
  run_privileged chmod -R u+rwX,go-rwx "$CUSTOM_ADDONS_DIR" 2>/dev/null || true

  if [[ "$CUSTOM_ADDONS_DIR" == "$ARTIFACTS_DIR" || "$CUSTOM_ADDONS_DIR" == "$ARTIFACTS_DIR/"* ]]; then
      run_privileged chown "$service_user:$service_group" "$ARTIFACTS_DIR" 2>/dev/null || true
      run_privileged chmod 750 "$ARTIFACTS_DIR" 2>/dev/null || true
  fi
  fi
...
create_systemd_service() {
  ...
  *)
  ...
  run_privileged chown -R "$service_user:$service_group" "$ROOT" 2>/dev/null || true
  fi

  service_tmp="$(mktemp "$RESTORE_WORKDIR/odoo-cs.service.XXXXXX")"
  cat > "$service_tmp" << EOF
[Unit]
...
EOF
  ...
  run_privileged install -m 0644 "$service_tmp" "$service_file"
  rm -f "$service_tmp"
  run_privileged systemctl daemon-reload
  run_privileged systemctl enable odoo-cs 2>/dev/null || true

  log_info "Systemd service created and enabled: odoo-cs.service"
}
...
disable_conflicting_services() {
  ...
  run_privileged systemctl stop odoo.service 2>/dev/null || true
  run_privileged systemctl disable odoo.service 2>/dev/null || true
...
run_post_restore_hooks() {
  ...
  # 7. Create systemd service (Linux with systemd)
